...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        double percent_rate = (100+rate)/100;
        for (; targetYear > THIS_YEAR ; targetYear--) {
            presentValue = percent_rate * presentValue;
        }
        return presentValue;
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return futureValue(futureValue(presentValue, rate, targetYear), -inflationRate, targetYear);
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        for (int k = targetYear; k > THIS_YEAR; k--) {
            totalMoneyInvested += futureValue(perYear, rate, k);
        }
        totalMoneyInvested += perYear;
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double final_rate = 1;
        double percent_inflationRate = (100- inflationRate)/100;
        for (int k = targetYear; k > THIS_YEAR; k--) {
            final_rate *= percent_inflationRate;
        }

        return round(totalSavings(perYear, targetYear, rate)* final_rate);
