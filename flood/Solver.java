...
class Solver {
    ...
    static int movesNeeded(Model model) {
        ...
        while (!work.solved()) {
            num += 1;
            Solver.chooser(work, SEARCH_DEPTH);
    ...
    static int[] chooser(Model model, int depth) {
        ...
        int currentActiveSize = m.activeRegionSize();
        for (int k = 0; k < m.ncolors(); k++) {
            m = new Model(model);


            m.setActiveRegionColor(k);
            maxSize = m.activeRegionSize();

            if (currentActiveSize < maxSize) {
                currentActiveSize = maxSize;
                bestMove = k;
            }

            if (winningScore == currentActiveSize) {
                bestMove = k;
                return new int[] { bestMove, currentActiveSize };
            }
        ...
        /**
        int color = 1;
        numberAdded = model.adjacentCells(color).size();

        for (color = 1; color < model.ncolors(); color++) {
            if (model.adjacentCells(color).size() > numberAdded) {
                numberAdded = model.adjacentCells(color).size();
            }
        ...
        /**
        maxSize = model.activeRegionSize();
        for (int color = 0; color < model.ncolors(); color++) {
            model.setActiveRegionColor(color);
            if (maxSize < model.activeRegionSize()) {
                maxSize = model.activeRegionSize();
                bestMove = color;
            }
            model.undo();
