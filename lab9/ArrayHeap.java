...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        ...
        if (getNode(index2) != null && getNode(index1) == null){
            return index2;
        }
        else {
            return 0;
    ...
    private void bubbleUp(int index) {
        ...
        if (getNode(parentIndex) != null) {
            if (getNode(parentIndex)._priority > getNode(index)._priority) {
    ...
    private void bubbleDown(int index) {
        int minimumIndex = min(getLeftOf(index), getRightOf(index));
        if (minimumIndex != 0) {
            if (getNode(minimumIndex)._priority < getNode(index)._priority) {
                swap(minimumIndex, index);
                bubbleDown(minimumIndex);
            }
        }
        /**
        if (getLeftOf(index) <= size() && getNode(getLeftOf(index))._priority < getNode(index)._priority) {
            swap(getLeftOf(index), index);
            bubbleDown(getLeftOf(index));
        }
        else if (getRightOf(index) <= size() && getNode(getRightOf(index))._priority < getNode(index)._priority) {
    ...
    public void insert(T item, double priority) {
        ...
        int newIndex = size() + 1;
        setNode(newIndex, newNode);
        bubbleUp(newIndex);

        /**int i = 1;
        while (i < size()) {
            if (getNode(newIndex)._priority < getNode(i)._priority) {
                swap(newIndex, i);
            }
            i = min(getLeftOf(i), getRightOf(i));
            bubbleDown(i)
        }
         */
    }
    ...
    public T removeMin() {
        ...
        int pointer = 1;
        swap(1, size());
        int minimumIndex = min(getLeftOf(pointer), getRightOf(pointer));


        /**while (pointer <= size() && minimumIndex != 0) {
            if (getNode(pointer)._priority > getNode(minimumIndex)._priority) {
                swap(pointer, minimumIndex);
                pointer = minimumIndex;
            }
        }

         */
        T save = removeNode(size())._item;
        bubbleDown(1);
        return save;
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size(); i++) {
           if (getNode(i)._item.equals(item)) {
               getNode(i)._priority = priority;

               if (getNode(i)._priority < getNode(getParentOf(i))._priority) {
                   bubbleUp(i);
               } else {
                   bubbleDown(i);
               }
