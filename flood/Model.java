...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        int row = place.row;
        int col = place.col;
        if (_cells.length > row + 1) {
            action.accept(pl(row + 1, col));
        }
        if (_cells[0].length > col + 1) {
            action.accept(pl(row, col + 1));
        }
    ...
    boolean[] colorsPresent() {
        int maxNumColors = _ncolors;
        boolean[] colorsPresentArray = new boolean[maxNumColors];

        for (int index = 0; index < colorsPresentArray.length; index++) {
            colorsPresentArray[index] = false;
        }
        for (int r = 0; _cells.length > r; r++) {
            for (int c = 0; _cells[0].length > c; c++) {
                int color = _cells[r][c];
                if (color >= 0 && color < _ncolors) {
                    if (!colorsPresentArray[color]) {
                        colorsPresentArray[color] = true;
                    }
                }
    ...
    void setActiveRegionColor(int newColor) {
        HashSet<Place> newRegion = new HashSet<>();
        GameState newState = new GameState();
        Place newPlace = pl(0, 0);
        for (Place cell : _active) {
            _cells[cell.row][cell.col] = newColor;

        }
        _active = findRegion(newPlace, newColor, newRegion);
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> oldRegion = new HashSet<>(_active);


        setActiveRegionColor(color);
        HashSet<Place> newRegion = new HashSet<>(_active);

        for (Place cell : oldRegion) {
            newRegion.remove(cell);
        }
